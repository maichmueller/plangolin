...
def get_enc_initial_and_goal(hidden_size=1, problem: str = "small"):
    ...
    encoder = HeteroEncoding(domain, hidden_size)
...
def test_fan_out():
    ...
    d1[encoder.obj_name].x = torch.tensor([[1.0], [2.0]])
    d2[encoder.obj_name].x = torch.tensor([[3.0], [4.0]])
    ...
    mlps = dict()
    for pred, arity in encoder.arity_by_pred.items():
        mlps[pred] = mlp_mock

    fan_out = FanOutMP(mlps, encoder.obj_name)
    ...
    on_g_encoding = out["on" + encoder.goal_suffix]
    ...
    assert encoder.obj_name not in out
...
def test_fan_in():
    ...
    # Make objs clearly distinguishable
    batch = Batch.from_data_list([d1, d2])
    batch["on" + encoder.goal_suffix].x = torch.tensor([[1.0, 2.0], [3.0, 4.0]])
    batch["on"].x = torch.tensor([[3.0, 4.0]])
    batch["clear"].x = torch.tensor([[1.0], [2.0], [3.0]])
    batch["ontable"].x = torch.tensor([[1.0], [2.0], [4.0]])

    fan_out = FanInMP(hidden, encoder.obj_name)
    # Filter out empty edge_types (also includes handempty() and holding(x))
    edge_index_dict = {k: v for k, v in batch.edge_index_dict.items() if v.numel() != 0}
    out: dict[str, torch.Tensor] = fan_out(batch.x_dict, edge_index_dict)
    assert len(out.keys()) == 1
    assert encoder.obj_name in out
    obj_embeddings = out[encoder.obj_name]
