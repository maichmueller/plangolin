...
class ExpandedStateSpaceEnv(EnvBase):
    ...
    def rand_action(self, tensordict: Optional[TensorDictBase] = None):
        ...
        batched_transitions = tensordict["transitions"]
        assert isinstance(batched_transitions, List)

        tensordict["action"] = as_non_tensor_stack(
            [
                transitions[
    ...
    def _reset(self, td: TensorDict, **kwargs) -> TensorDict:
        ...
        out = TensorDict(
            {
                "state": as_non_tensor_stack(self.current_states),
                "transitions": as_non_tensor_stack([initial_transitions] * batch_size),
                "goal": as_non_tensor_stack([self.problem.goal] * batch_size),
            },
    ...
    def _step(self, td: TensorDict) -> TensorDict:
        ...
        out = TensorDict(
            {
                "state": as_non_tensor_stack(self.current_states),
                "transitions": as_non_tensor_stack(self.get_applicable_transitions()),
                "goal": td.get("goal"),
                "reward": reward,
                "done": done,
            },
